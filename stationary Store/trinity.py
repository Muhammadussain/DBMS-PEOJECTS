...
class menu:
    ...
    def entry(self):
        ...
        l1=Button(self.a,text='Add Task',font='Arial 22 bold',fg='wheat1',bg='salmon4',width=15,padx=10,command=self.addentry).place(x=20,y=700)
        l2=Button(self.a,text='Generate Report',font='Arial 22 bold',fg='wheat1',bg='salmon4',width=15,padx=10,command=self.search).place(x=420,y=700)

        l4=Button(self.a,text='All Task',font='Arial 22 bold',fg='wheat1',bg='salmon4',width=15,padx=10,command=self.all).place(x=788,y=700)
        l4=Button(self.a,text='EXIT',font='Arial 22 bold',fg='wheat1',bg='salmon4',width=15,padx=10,command=self.mainmenu).place(x=1156,y=700)
    ...
    def addentry(self):
        ...
        l1=Label(self.f1,text='Product ID : ',font='Arial 12 bold',fg='salmon4',bg='wheat1',pady=1).place(x=50,y=50)
        ...
        l3=Label(self.f1,text='Client : ',font='Arial 12 bold',fg='salmon4',bg='wheat1',pady=1).place(x=50,y=150)
        e3=Entry(self.f1,width=30,bg='wheat1',font='Arial 12 bold',fg='black',textvariable=self.title).place(x=200,y=150)
        l4=Label(self.f1,text='Price : ',font='Arial 12 bold',fg='salmon4',bg='wheat1',pady=1).place(x=50,y=200)
    ...
    def search(self):
        ...
        self.f1=Frame(self.a,height=500,width=650,bg='salmon4')
        self.f1.place(x=500,y=100)
        l1=Label(self.f1,text='Product ID/Title/Client: ',font=('Arial 10 bold'),bd=2, fg='salmon4',bg='wheat1').place(x=20,y=40)
    ...
    def serch1(self):
        k=self.sid.get()
        if k!="":
            self.list4=("Product ID","TITLE","CLIENT","PRICE","Quantity","COMMENT")
            self.trees=self.create_tree(self.f1,self.list4)
            self.trees.place(x=25,y=150)
            conn=sqlite3.connect('press.db')

            c=conn.execute("select * from entries where id=? OR TITLE=? OR CLIENT=?",(k.capitalize(),k.capitalize(),k.capitalize(),))
            a=c.fetchall()
            if len(a)!=0:
                for row in a:
    ...
    def all(self):
        ...
        conn=sqlite3.connect('press.db')
        self.list3=("Product ID","TITLE","Client","Price","Quantity","Comment")
...
def Database():
    ...
    cursor.execute("SELECT * FROM `login` WHERE `username` = 'admin' AND `password` = 'hussain'")
    if cursor.fetchone() is None:
        cursor.execute("INSERT INTO `login` (username, password) VALUES('admin', 'hussain')")
        conn.commit()
...
PASSWORD = StringVar()
#==============================LABELS=========================================
lbl_title = Label(canvas, text = "Stationary Store Management Sysem", font=('Arial', 30,'bold', ),bg='PeachPuff4', fg='black')
